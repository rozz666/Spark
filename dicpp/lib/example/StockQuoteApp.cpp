...
StockQuoteApp::StockQuoteApp(boost::shared_ptr<UserInterface> ui,
...
StockQuoteApp::loop()
{
  ...
  boost::shared_ptr<StockQuoteService> service = m_factory->createService( result.second );
...
{
  m_ui->execute(boost::bind(&StockQuoteApp::loop,this));
}

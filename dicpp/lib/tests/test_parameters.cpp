...
{
  BOOST_MPL_ASSERT(( boost::is_same<boost::shared_ptr<int>,
                     di::traits::returned_type<boost::shared_ptr<int> >::type> ));
...
#define GEN(N)                                                          \
  ...
  {                                                                     \
    BOOST_PP_CAT(Arg,N)(                                                \
      BOOST_PP_ENUM_PARAMS(N,boost::shared_ptr<Parameter> p),           \
      boost::shared_ptr<BOOST_PP_CAT(Arg,BOOST_PP_DEC(N))>              \
      ){}                                                               \
  };                                                                    \
                                                                        \
  namespace di                                                          \
  {                                                                     \
    ...
    {                                                                   \
      template<>                                                        \
        struct type_meta<BOOST_PP_CAT(Arg,N)>                           \
      {                                                                 \
...
BOOST_AUTO_TEST_CASE( NumberOfParameters )
{
  ...
  boost::shared_ptr<max_args> instance = r.construct<boost::shared_ptr<max_args> >();

...
BOOST_AUTO_TEST_CASE( ScopedProvider )
{
  ...
  boost::shared_ptr<di::typed_provider<AnObject> > provider =
    r.construct<boost::shared_ptr<di::typed_provider<AnObject> > >();
  ...
  // Get the instance
  boost::shared_ptr<AnObject> object = provider->get();

  // Should be one instance
  BOOST_REQUIRE_EQUAL ( obj_instances, 1U);

  boost::shared_ptr<AnObject> object2 = provider->get();
